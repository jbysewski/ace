...
var DOMRenderer = require("./virtual_renderer").VirtualRenderer;
var WebGLRenderer = require("./virtual_renderer_webgl").VirtualRenderer;
...
(function(){
    ...
    this.$createEditor = function() {
        ...
        if(this.$useWebGLRenderer)
            renderer = new WebGLRenderer(el, this.$theme);
        else
            renderer = new DOMRenderer(el, this.$theme);
    ...
    this.setUseWebGLRenderer = function(useWebGLRenderer) {
        if(this.$useWebGLRenderer !== useWebGLRenderer) {
            this.$useWebGLRenderer = useWebGLRenderer;
            var splits = this.$splits;
            this.$removeSplits(0);
            var sessions = [];
            var editor;
            while((editor = this.$editors.pop()) !== undefined) {
                sessions.push(editor.getSession());
            }
            this.setSplits(splits);
            for(var i=0; i<this.$editors.length; i++) {
                this.$editors[i].setSession(sessions[i]);
            }
        }
    };

    this.getUseWebGLRenderer = function() {
        return this.$useWebGLRenderer;
    };
    ...
    this.setSplits = function(splits) {
        ...
        if (splits == this.$splits) {
            ...
            this.$removeSplits(splits);
        }
        this.resize();
    };

    this.$removeSplits = function(target, removeEditor) {
        while (this.$splits > target) {
            var editor = this.$editors[this.$splits - 1];
