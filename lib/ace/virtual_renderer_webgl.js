...
var VirtualRenderer = function(container, theme) {
    ...
    webgl.element.className = "ace_layer ace_content";
    this.container.appendChild(webgl.element);

    this.$hidden = document.createElement("div");    
    this.$hidden.className = "ace_scroller";
    webgl.element.appendChild(this.$hidden);
    ...
    this.$gutterLayer = new WebGLGutterLayer(webgl);
    this.$gutterLayer.on("changeGutterWidth", this.onResize.bind(this, true));
    this.setFadeFoldWidgets(false);

    this.$markerBack = new MarkerLayerWebGL(webgl);

    var textLayer = this.$textLayer = new WebGLTextLayer(webgl);

    this.$markerFront = new MarkerLayerWebGL(webgl);

    this.characterWidth = textLayer.getCharacterWidth();
    this.lineHeight = textLayer.getLineHeight();

    this.$cursorLayer = new CursorWebGLLayer(webgl);
    ...
    **/
    this.setTheme = function(theme) {
        ...
        function afterLoad(theme) {
            ...
            var b = dom.computedStyle(_self.$hidden, "background-color").replace(/.*rgb\(/, "").replace("rgba(", "").replace(/\).*/, "").split(",");
            if(b.length == 4)
