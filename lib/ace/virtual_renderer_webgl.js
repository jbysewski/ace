...
var VirtualRenderer = function(container, theme) {
    ...
    var teflon = this.$teflon = new Teflon(container);
    teflon.canvas.cssClassName = "ace_layer ace_content";

    this.$palette = this.createDefaultPalette();
    this.$font = teflon.createFont();
    this.$textViewer = teflon.createTextViewer([this.$font, this.$font], this.$palette, {inputHandling : false});
    this.$text = this.$textViewer.text;
    this.$cursor = this.$textViewer.cursor;

    this.$hiddenBg = document.createElement("div");    
    this.$hiddenBg.className = "ace_scroller";
    teflon.canvas.appendChild(this.$hiddenBg);

    this.$gutter = dom.createElement("div");
    this.$gutter.className = "ace_gutter";

    var textParent = document.createElement("div");
    textParent.className = "ace_text-layer ace_line";
    this.$hiddenText = document.createElement("span");    
    textParent.appendChild(this.$hiddenText);
    teflon.canvas.appendChild(textParent);
...
(function() {
    ...
    **/
    ...
    this.$loadDocument =function() {
        ...
        for(var row=0; row<rows; row++) {
            ...
            for(var i=0; i<tokens.length; i++) {
                var token = tokens[i];
                var color = this.$extractTextColor(token);

                this.$textViewer.text.addColorToken(token.value, col, color);
        ...
        var self = this;
        this.session.getDocument().addEventListener("change", function(e) { self.$onDocumentChange(e); });
    };

    this.$extractTextColor = function(token) {
        if(token.type in this.$textColorCache) {
            return this.$textColorCache[token.type];
        }

        this.$hiddenText.className = "ace_" + token.type.replace(/\./g, " ace_");                
        var style = dom.computedStyle(this.$hiddenText, "color");
        return this.$textColorCache[token.type] = this.$palette.colors[style];
    };
    this.$clearTextColorCache = function() { this.$textColorCache = {} }

    this.$overwriteRows = function(sRow, eRow) {
        for(var row=sRow; row<=eRow; row++) {
            ...
            for(var i=0; i<tokens.length; i++) {
                ...
                this.$textViewer.text.overwriteColorToken(token.value, row, col, color);
                col += token.value.length;
            }
        }
    }
    ...
    this.$onDocumentChange = function(e) {
        console.log(e);
        var data = e.data;
        if(data.action == "removeText" || data.action == "insertText") {
            this.$text.clearRows(data.range.start.row, data.range.end.row);
            this.$overwriteRows(data.range.start.row, data.range.end.row);
        }
        this.$textViewer.repaint();
    };
    ...
    **/
    ...
    this.$moveTextAreaToCursor = function() {
        ...
        var posTop = this.$cursor.row * this.$font.rowSize;
        var posLeft = this.$cursor.col * this.$font.colSize;
        posTop -= this.$text.scroll;

        if (posTop < 0 || posTop > this.$teflon.canvas.height)
            return;
    ...
    **/
    this.scrollToRow = function(row) {
        this.session.setScrollTop(row * this.$font.rowSize);
    };
    ...
    **/
    this.scrollToY = function(scrollTop) {
        var row = scrollTop / this.$font.rowSize;
        if (this.$text.scroll !== row) {
            this.$text.scroll = row;
            this.$textViewer.repaint();
        }
    };
    ...
    **/
    ...
    this.pixelToScreenCoordinates = function(x, y) {
        ...
        var offset = (x - canvasPos.left - this.$padding - this.$text.pan) / this.$font.rowSize;
        var row = Math.floor((y + this.$text.scroll - canvasPos.top) / this.$font.colSize);
    ...
    this.screenToTextCoordinates = function(x, y) {
        ...
        var col = Math.round(
            (x - canvasPos.left - this.$text.pan) / this.$font.colSize
        );
        var row = Math.floor(
            (y + this.$text.scroll * this.$font.rowSize - canvasPos.top) / this.$font.rowSize
        );
    ...
    **/
    this.textToScreenCoordinates = function(row, column) {
        ...
        var x = this.$padding + Math.round(pos.column * this.$font.colSize);
        var y = pos.row * this.$font.rowSize;

        return {
            pageX: canvasPos.left + x - this.$text.pan - this.layerConfig.xOffset,
            pageY: canvasPos.top + y - this.$text.scroll
        };
    };
    ...
    **/
    this.setTheme = function(theme) {
        ...
        function afterLoad(theme) {
            ...
            var b = dom.computedStyle(_self.$hiddenBg, "background-color");
            if(b.length == 16)
