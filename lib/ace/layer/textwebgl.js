...
(function() {
    ...
    this.update = function(config) {
        ...
        while (true) {
            ...
            var base = (row - firstRow) * this.$font.$cols;
            var column = 0;
            for(var i=0; i<tokens.length; i++) {
                ...
                for(var c=0; c<token.value.length; c++) {
                    indexes[base + column] = ((token.value.charCodeAt(c) - 32) << 18) + color;
                    column++;
                }
            }        
    ...
    this.$extractColor = function(token) {
        ...
        if(r == 255 && g == 255 && b == 255) {
            // 'white' is returned as color, when no color is set -> use black
            r = b = g = 0.0;
        }
        ...
        var color = (r << 12) + (g << 6) + (b);
        this.$colorCache[token.type] = color;
        return color;
    };
