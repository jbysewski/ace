...
(function() {
        ...
        while (true) {
            ...
            for(var i=0; i<tokens.length; i++) {
                ...
                for(var c=0; c<token.value.length; c++) {
                    this.$indexes[base + column] = this.$font.packChar(token.value.charCodeAt(c)) + color;
                    column++;
                }
            }        
    ...
    this.draw = function() {
        ...
        this.$font.draw(this.$indexes, this.config.xOffset, -this.config.yOffset + 1, this.config.upsidedown);
    }
