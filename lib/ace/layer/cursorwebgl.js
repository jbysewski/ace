...
(function() {
    ...
    this.update = function(config) {
        ...
        if (this.session.selectionMarkerCount > 0) {
            ...
            for (var i = selections.length; i--; ) {
                ...
                this.$drawCursor(cursor, pixelPos, config);
            }
        } else {
    ...
    this.$drawCursor = function(cursor, pos, config) {
        if(cursor.width !== config.characterWidth || cursor.height !== config.lineHeight)
                cursor.updateSize(2, config.lineHeight);

        cursor.draw(pos.left + config.xOffset, pos.top - config.yOffset, 0.0, 0.0, 0.0, 0.8);
    }
